...
if ERRORLEVEL 1 (
    echo.
    echo FAILURE DETECTED
    echo Could not delete appgui.spec
    goto ERROR
)
...
call move version.dat temp_zip\appgui
:: Detect if failure happened while running the above
if ERRORLEVEL 1 (
    echo.
    echo FAILURE DETECTED
    echo Could not move version.dat into temp_zip\appgui directory
    goto ERROR
)

:: Run small script to crate a zipped folder (named appgui - see zip.py), contatining the contents of the temporary folder
